...
namespace json::detail {
	...
	private:
		...
		constexpr std::pair<char32_t, bool> fParseEscape() {
			...
			case U'u':
				break;
			default:
				fParseError("Unknown escape-sequence in string encountered");
			}
			...
			for (size_t j = 0; j < 4; ++j) {
				...
				if (val >= 16)
					fParseError("Invalid [\\u] escape-sequence in string encountered");
				num = (num << 4) + val;
			}
	...
	private:
		constexpr void fUnexpectedToken(char32_t token, const char* expected) {
			std::string err = str::ch::Format("Unexpected token [{:e}] encountered at {} when {} was expected",
				token, pPosition, expected);
			throw json::DeserializeException{ err };
		}
		constexpr void fParseError(const char* what) {
			throw json::DeserializeException{ what, " while parsing the json at ", pPosition };
		}
		constexpr void fCheckWord(std::u32string_view word) {
			...
			for (size_t i = 1; i < word.size(); ++i) {
				char32_t c = fConsumeAndNext(false);
				if (c != word[i])
					fUnexpectedToken(c, str::ch::Format("[{}] of [{}]", word[i], word).c_str());
			}
			fConsume();
		}
	...
	public:
		constexpr bool closeElseSeparator(bool obj) {
			...
			/* setup the error */
			fUnexpectedToken(c, obj ? "[,] or closing object-bracket" : "[,] or closing array-bracket");
			return false;
		}
		...
		constexpr detail::NumberValue readNumber() {
			...
			if (state == NumState::preSign || state == NumState::preDigits || state == NumState::preFraction || state == NumState::preExpSign || state == NumState::preExponent)
				fParseError("Malformed json number encountered");
			...
			if (result.consumed != pBuffer.size())
				fParseError("Number parsing error occurred");
			return value;
		}
		constexpr void readString(auto& sink, bool key) {
			...
			char32_t c = fNextToken(true);
			if (c != U'\"') {
				fUnexpectedToken(c, "[\"] as start of a string");
				return;
			}
			...
			while (true) {
				...
				if (c == U'\"') {
					...
					c = fNextToken(true);
					if (c != U':')
						fUnexpectedToken(c, "[:] object-separator");
					else
						fConsume();
					return;
				}
				...
				if (cp::prop::IsControl(c))
					fParseError("Control characters in string encountered");
				if (c != U'\\') {
